...
double minkowski(NumericVector x, NumericVector y, double p){
  // calculates the Minkowski distance between two points for a given p
  double distance;
  if (p > 0){
...
// [[Rcpp::export]]
int my_knn_c_minkowski( NumericMatrix X, NumericVector X0, NumericVector y, 
                        double p){
  // Performs kNN (with k=1) classification with Minkowski distance
  // for a given p´
  // 
  // X: data matrix with input attributes
  // y: response variable values of instances in X  
...
int my_knn_tuningp(NumericMatrix X, NumericVector X0, NumericVector y,
  ...
  // Given a vector of Minkowski parameters, studies the performance of
  ...
  for (int i=0;i<possible_p.size();i++){
    ...
    // predict with Minkowski kNN the label for each element in the test set
