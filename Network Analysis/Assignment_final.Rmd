...
EN EL ASSIGNMENT 3 HAY QUE AÑADIR EN EL GENERAL EL SECOND FIT OUT
...
In this final section, we assess whether out congresspeople network has the small world property. Firstly, we are going to use our network to generate a large number of random networks with the same number of vertices and edges than those of our congresspeople network. Then, for each network we obtain the clustering coefficient and the average path length.
...
Next, we compare the results obtained with those of our network. In this case, note that the average path length of the karate network is 2.0639 which is in the middle of the histogram, which it is fine. However, the clustering coefficient is 0.2695 which is larger than the standard clustering coefficient of the networks with the same number of vertices and edges, which suggests that the karate network is closer to owning the small world property than networks with the same characteristics.

CORRECT THIS BECAUSE IT IS NOT IN THE CENTER OF THE HISTOGRAM THE FIRST NUMBER
